...
class ToastSystem::Worker: public UpdateSystem::Worker
{
public:
	Worker(ToastSystem* toastSystem);
	bool IsFinished() override;
	void Update(uint64_t delta) override;
private:
	ToastSystem* m_pToastSystem;
...
void ToastSystem::AddMessage(const wxString& msg, wxWindowID id, bool preventSameMessage)
{
	...
	{
		m_pWorker = new ToastSystem::Worker(this);
		wxGetApp().GetUpdateSystem().AddWorker(m_pWorker);
	}
}
...
ToastSystem::Worker::Worker(ToastSystem* toastSystem)
	: m_pToastSystem(toastSystem)
{
}

bool ToastSystem::Worker::IsFinished()
{
	const bool isFinished = m_pToastSystem->m_messages.empty() || m_pToastSystem->m_pTargetWindow->IsBeingDeleted();
	if(isFinished)
	{
		m_pToastSystem->m_pWorker = nullptr;
	}
	
	return isFinished;
}

void ToastSystem::Worker::Update(uint64_t delta)
{
	m_pToastSystem->RemoveFinishedToasts();
	for ( auto& it : m_pToastSystem->m_messages )
	{
		it.Update(delta);
	}

	m_pToastSystem->m_pTargetWindow->TryRender();
}
